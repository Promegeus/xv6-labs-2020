...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    // 如果该页本身就是不可写(只读)，则不会添加这个标志位（只读的页一直是父子进程共享的）
    if(*pte & PTE_W)
    ...
    // （现在都是不可写的，原本可写的页会新增PTE_COW写时复制标志）
    if(mappages(new, i, PGSIZE, (uint64)pa, flags) != 0)
      goto err;

    my_krefpage((void*)pa);   // 映射的物理页的引用次数+1
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    ...
    // 检查每一个页是否为COW页，如果是的话就执行复制操作
    if(my_uvmcheckcowpage(dstva))
      my_uvmcowcopy(dstva);
...
int my_uvmcheckcowpage(uint64 va)
{
  ...
  return va < p->sz
        ...
        && (*pte & PTE_V)
        && (*pte & PTE_COW);
  // 地址在进程内存范围内 && 地址有映射 && 地址有效 && 所在页是COW页
}
...
int my_uvmcowcopy(uint64 va)
{
  ...
  // 获取映射的物理地址
  uint64 pa = PTE2PA(*pte);   
  // 获取新分配的物理页的物理地址，把原本物理页引用次数-1
  //  (如果原本物理页引用数为1，则获取到的还是原本的物理页，因为也没有其他进程使用这个物理页了)
