...
int pageref[PGREF_MAX_ENTRIES];   // 每个物理页的引用数(pageref[i]表示第i个物理页的引用数)
struct spinlock pgreflock;        // 用于pageref数组的锁，防止竞态条件引起的内存泄漏

#define PA2PGREF(p) pageref[PA2PGREF_ID((uint64)(p))]   // 获取物理地址对应物理页的引用数
...
kalloc(void)
{
  ...
  if(r)
  {
    ...
    PA2PGREF(r) = 1;    // 将新分配的物理页的引用数设置为1(刚分配的页还没映射，不会有进程来用，不用加锁) 
  }
  return (void*)r;
}
...
// 如果引用数<=1，则无需操作，直接返回该物理页
void* my_kcopy_n_deref(void* pa)
{
  acquire(&pgreflock);

  // 若当前物理页引用次数为1，无需分配新的物理页
  if(PA2PGREF(pa) <= 1)
  {
    release(&pgreflock);
    return pa;
